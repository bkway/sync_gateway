...
func (bucket *CouchbaseBucketGoCB) Query(statement string, params map[string]interface{}, consistency ConsistencyMode, adhoc bool) (results sgbucket.QueryResultIterator, err error) {
	...
	for i := 1; i <= MaxQueryRetries; i++ {
		...
		logger.For(logger.QueryKey).Trace().Msgf("Executing N1QL query: %v", logger.UD(n1qlQuery))
		...
		err = queryErr
		logger.For(logger.QueryKey).Warn().Err(err).Msgf("Indexer error during query - retry %d/%d after %v.", i, MaxQueryRetries, waitTime)
		time.Sleep(waitTime)

		waitTime = time.Duration(waitTime * 2)
	}

	logger.For(logger.QueryKey).Err(err).Msgf("Exceeded max retries for query when querying index using statement: [%s]", logger.UD(bucketStatement))
	return nil, err
}
